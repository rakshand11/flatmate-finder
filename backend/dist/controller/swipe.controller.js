...
export const swipeUser = async (req, res) => {
    try {
        ...
        const alreadySwiped = await pool.query('SELECT id FROM swipes WHERE swiper_id=$1 AND swiped_id=$2', [swiper_id, swiped_id]);
        ...
        await pool.query('INSERT INTO swipes(swiper_id, swiped_id, direction) VALUES ($1, $2, $3)', [swiper_id, swiped_id, direction]);
        // if left swipe → stop here
        if (direction === "left") {
            return res.status(200).json({ msg: "Swiped left" });
        }
        // check mutual swipe (FIXED quotes)
        const mutualSwipe = await pool.query('SELECT id FROM swipes WHERE swiper_id=$1 AND swiped_id=$2 AND direction=$3', [swiped_id, swiper_id, "right"]);
        ...
        const existingMatch = await pool.query('SELECT id FROM matches WHERE user1_id=$1 AND user2_id=$2', [user1_id, user2_id]);
        if (existingMatch.rows.length > 0) {
            return res.status(200).json({ msg: "Already matched" });
        }
        // create match
        const match = await pool.query('INSERT INTO matches (user1_id, user2_id) VALUES ($1, $2) RETURNING *', [user1_id, user2_id]);
